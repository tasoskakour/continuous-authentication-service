...
var KEYSTROKE_DATA_LENGTH_LIMIT_BACKUP = 20;
...
var cont_auth_server_get_random_sentence = cont_auth_server_url + '/misc/' + 'get-random-sentence-safeshop'; // + '/:MAXIMUM_LENGTH
 ...
 */
function sendData(force = false, byPassTest = false) {
            ...
            function (data) {
                ...
                if (res.alert == true && loggedInFlag) {
                    if (res.isImpostor == true) {
                        ...
                        if (!insideImpostorModalFlag) {
                            resetKeystrokeData();
                            KEYSTROKE_DATA_LENGTH_LIMIT = 50;
                    ...
                    if (!insideImpostorModalFlag) {
                        ...
                        KEYSTROKE_DATA_LENGTH_LIMIT = KEYSTROKE_DATA_LENGTH_LIMIT_BACKUP;
                        deactivateImpostorModal();
                        insideImpostorModalFlag = false;
                        activateUserRetakeSuccessModal();
                        // Send what was maybe left in the buffer from the retake sentence input
                        if (keystrokeData.length > 0) {
                            sendData(force = true, byPassTest = true);
                        }
